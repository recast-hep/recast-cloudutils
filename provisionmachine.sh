...
#!/bin/zsh
export RECAST_PLUGINHOMEDIR=plugin_home
...
EOF
chown recast:recast user_data.sh
...
EOF
unzip auth.zip
mv recast-auth-resources/dot-ssh /home/recast/.ssh/
chown -R recast:recast /home/recast/.ssh
...
rm -f /home/recast/recast_auth/privkey.pem
rm -f /home/recast/recast_auth/host.cert
...
chcon -Rv --type=cvmfs_cache_t /var/cache/cvmfs &> /dev/null

EOFOUT

echo "::::wrote user_data to $tmpfile"

echo nova boot --image $image --flavor $flavor --key_name openstack --user_data $tmpfile $hostname --meta landb-os=linux --meta landb-osversion=SLC6
nova boot --image $image --flavor $flavor --key_name openstack --user_data $tmpfile $hostname --meta landb-os=linux --meta landb-osversion=SLC6
